  ...
  const Service = rest.service(function() {
    ...
    static: function(id, noBasemap) {
      ...
      this.viz(id, false).then(viz => {
        ...
        ]).spread((layers, named_map) => {
          // Instanciate a named map to gets its layergroupid
          this.instanciate(named_map.name).then(instance => {
            ...
            this.json('POST', 'v1/map/', { layers }).on('complete', result => {
    ...
    image: function(specs, width, height, protocol, format, useCenter) {
      // Set default values
