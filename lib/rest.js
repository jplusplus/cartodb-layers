...
class Rest {
  constructor ({ user, api_key }) {
    ...
    this.user = user || "cartodb";
    this.axios = axios.create({
      // Base URL is scoped to the current user
      baseURL: `https://${this.user}.carto.com/api/`
    });
  }
  get () {
  ...
  // Gets a NamedMap
  instanciate (name, data) {
    return this.post('/v1/map/named/' + name, data, {
  ...
  async projection (id, name, cartocss, sql, interactivity) {
    // Create a hash for the viz id
    const hash = crypto.createHash("sha256").update(id).digest('hex');
    // Timestamp token
    const now = ~~(Date.now()/1000);
    // Default name uses viz id
    name = name || ["tpl", now, hash].join('_');
    ...
    return this.delete(`/v1/map/named/${name}`, { params }).finally(() => {
      // Get the visualization details
      return this.post('/v1/map/named', data, {
        params,
        headers: {
          'Content-Type': 'application/json'
        }
      }).catch(() => {
        return { named_map: data, viz };
      })
    })
  }
  ...
  async static (id, noBasemap) {
    ...
    // Get layers
    const layers = await Q.Promise(resolve => {
    ...
    // Get template name
    const named_map = await Q.Promise(resolve => {
    ...
    const instance = await this.instanciate(named_map.name)
    // Add the mapnik to the list of layers
    layers.push({
      type: 'http',
      options: {
        // Build the ratserized map URL for the NamedMap instance
        urlTemplate: 'https://' + this.user + '.carto.com/api/v1/map/' + instance.layergroupid + '/{z}/{x}/{y}.png'
      }
    });
    // Get the static map attributes
    const result = this.post('/v1/map/', { params: this.buildParams() }, { layers })
  ...
  async vizSqlQuery (id) {
    ...
    // Attempt to find the query in the named map
    _.get(namedmapLayer, 'options.named_map.layers', []).forEach(({ layer_name }) => {
      sql = sql || `SELECT * FROM ${layer_name}`
    }, sql)
    return sql
  }
  fillParams (sql) {
