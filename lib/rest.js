...
module.exports = function(config) {
  const baseURL = "https://" + config.user + ".cartodb.com/api/";
  ...
  const Service = rest.service(function() {
    ...
    layers: function() {
      var query = this.buildQuery(this, arguments);
    ...
    viz: function(id, resolveNamedMap) {
      var query = this.buildQuery(this);
      var deferred = Q.defer();
      ...
      var that = this;
      // Get the viz
      this.get('v2/viz/' + id + '/viz.json', { query }).on('complete', function(viz) {
    ...
    projection: function(id, name, cartocss, sql, interactivity) {
      ...
      that.viz(id).then(function(viz) {
        var query = that.buildQuery(that);
        // Find a layergroup
