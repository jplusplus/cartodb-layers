...
class Rest extends Memoizable {
  ...
  async vizTable (id) {
    // Shortcut function to get the first non-null value of a map
    const firstMappedValue = (a, p) => _.chain(a).map(p).compact().first().value()
    // Get the visualization details
    let viz = await this.throttled(1000).getVizV2(id)
    let table_name = null;
    // Use named map to get the table name
    const named_map_layers = _.get(viz, 'layers.options.named_map.layers', [])
    table_name = table_name || firstMappedValue(named_map_layers, 'layer_name.sql')
    // Use related table to get the table name
    const { related_tables } = table_name ? [] : await this.throttled(1000).getVizV1(id)
    return table_name || firstMappedValue(related_tables, 'name')
  }
  fillParams (sql) {
