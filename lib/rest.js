...
module.exports = rest.service(function(hash) {
  ...
  projection: function(id, name, cartocss, sql) {
    ...
    that.viz(id).then(function(viz) {
      ...
      var cdbLayer = _.find(layergroup.options.layer_definition.layers, { type: 'cartodb' });
      // Build configuration
      var mapconfig = {
        "version": "0.0.1",
        "name": name,
        ...
        "layergroup": {
          "layers": [{
            ...
            "options": {
              ...
              "interactivity": cdbLayer.options.interactivity
            }
          }]
        }
      };
      ...
      that.del('v1/map/named/' + name, { query: query }).on('complete', function() {
        ...
        }).on('error', deferred.reject).on('fail', deferred.reject);
      });
    });
    // Returns the event emitter
    // which is resolved after a few requests
    ...
    that.viz(id, false).then(function(viz) {
      return Q.all([
        ...
        Q.Promise(function(resolve) {
          ...
          if( namedmap ) { return resolve(namedmap.options.named_map); }
          // No namedmap? We create one!
          that.projection(id).then(resolve);
        })
      ]).spread(function(layers, named) {
  ...
  fields: function(id) {
    ...
    that.projection(id).then(function(named) {
      ...
      named.layergroup.layers.forEach(function(layer) {
        sql = _.template(layer.options.sql)({ layer0: 1, layer1: 1, layer2: 1, layer3: 1 });
      });
      // No SQL avalaible
      if(!sql) {
